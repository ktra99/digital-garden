...
function Cookie(props: { className: string }) {
  return (
    <svg
      width="20"
      height="20"
...
function LanguageSwitch({ posts }: { posts: PostMeta[] }) {
  ...
  return (
    <div className="my-4 space-x-4 pl-[1.15rem] sm:my-0">
      <button
        type="button"
        ...
        className={clsx(
          locale === "en"
            ...
            : "text-zinc-900/40 sm:text-white/40",
          "font-sans text-xs font-bold uppercase sm:text-base"
        )}
        disabled={locale === "en"}
      >
        en
          ...
          <div className="hidden sm:flex sm:items-center sm:space-x-6">
            {navigation.map((item) => (
              ...
              >
                <item.icon
                  className={clsx(
                    item.finished ? "text-white" : "text-white/40",
                    "h-5 w-5"
                  )}
                />
              </button>
